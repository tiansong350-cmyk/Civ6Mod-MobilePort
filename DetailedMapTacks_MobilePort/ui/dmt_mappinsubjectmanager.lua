...
local memoryCache = {}; -- [DMT V38] In-Memory Cache for iPad
...
function LoadMapPinSubjectTable(playerID:number, key:string)
    -- [DMT V38] Try Memory Cache First (iPad Fix)
    if memoryCache[playerID] and memoryCache[playerID][key] then
        return memoryCache[playerID][key];
    end
...
function SaveMapPinSubjectTable(playerID:number, key:string, value:table)
    -- [DMT V38] Save to Memory Cache First (iPad Fix)
    if not memoryCache[playerID] then memoryCache[playerID] = {}; end
    memoryCache[playerID][key] = value;
