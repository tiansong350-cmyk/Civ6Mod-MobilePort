...
print("DMT: Version 21 Loaded (YieldCalculator Diagnostic)");
if Controls.DebugLabel then Controls.DebugLabel:SetText("DMT V21: YieldCalculator LUA LOADED"); end
...
function UpdatePinYields(playerID:number, pinsToUpdate:table)
    ...
    for _, pin in ipairs(pinsToUpdate) do
        -- [DMT V35] Debug & Safety Wrapper
        if Controls.DebugLabel then Controls.DebugLabel:SetText("Calc: " .. tostring(pin.Key)); end
        ...
        if not status then
            print("DMT CRASH in UpdatePinYields for " .. tostring(pin.Key) .. ": " .. tostring(err));
            if Controls.DebugLabel then Controls.DebugLabel:SetText("CRASH: " .. tostring(pin.Key)); end
        end
    end
...
function CreateMapPinSubject(mapPin:table)
    ...
    -- [DMT V29] Type Safety & Debug
    local rawType = type(iconName);
    local rawStr = tostring(iconName);
    
    if iconName then 
        iconName = string.upper(tostring(iconName)); 
    else
        iconName = "";
    end
    
    -- [DMT V31] Removed Pin0 debug to clear noise
    -- if Controls.DebugLabel and subject.Id == 0 then ... end
...
function OnMapPinRemoved(mapPinCfg:table)
    ...
    -- [DMT V39] Clear the removed pin from our cache
    ClearMapPinSubject(playerID, pin.X, pin.Y);

    -- [DMT V39] Force full recalculation of ALL pins when any pin is removed
    -- IMPORTANT: We must exclude the pin being deleted because at this point,
    -- the game data (PlayerConfigurations:GetMapPins()) hasn't been updated yet!
    -- The actual deletion happens AFTER this event fires.
    local deletedX = pin.X;
    local deletedY = pin.Y;
    
    local freshPinsToUpdate = {};
    local playerConfig = PlayerConfigurations[playerID];
    if playerConfig then
        local playerPins = playerConfig:GetMapPins();
        for _, mapPin in pairs(playerPins) do
            -- Skip the pin being deleted
            if mapPin:GetHexX() ~= deletedX or mapPin:GetHexY() ~= deletedY then
                local freshPin = CreateMapPinSubject(mapPin);
                if freshPin.Type ~= MAP_PIN_TYPES.UNKNOWN then
                    table.insert(freshPinsToUpdate, freshPin);
                end
            end
        end
    end
    
    UpdatePinYields(playerID, freshPinsToUpdate);
end
...
function OnLocalPlayerTurnBegin(isFirstLoad)
    ...
    -- [DMT V25] Debug
    if Controls.DebugLabel then Controls.DebugLabel:SetText("TurnBegin: Found " .. #allPins .. " pins"); end
