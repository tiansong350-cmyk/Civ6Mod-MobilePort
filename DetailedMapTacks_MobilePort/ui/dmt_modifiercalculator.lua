...
function CalculateDistrictYieldFromSingleModifier(modifierSubject:table, pinSubject:table, adjFeatures:table, adjYields:table, playerID:number)
    ...
    if effectType == "EFFECT_RIVER_ADJACENCY" then
        -- [DMT V34] Safe Map Plot Access
        local status, plot = pcall(Map.GetPlot, pinSubject.X, pinSubject.Y);
        if status and plot then
            local isRiver = false;
            pcall(function() isRiver = plot:IsRiver(); end); -- Wrap IsRiver
            
            if isRiver and PlayersVisibility[playerID] then -- Check PlayersVisibility
                local isRevealed = false;
                pcall(function() isRevealed = PlayersVisibility[playerID]:IsRevealed(plot:GetIndex()); end);
                if isRevealed then
    ...
    if effectType == "EFFECT_DISTRICT_ADJACENCY" then
        ...
        if adjList then
            ...
            -- Get yield.
            -- [DMT V34] Safe Arithmetic
            local amount = tonumber(arguments.Amount) or 0;
            result.Amount = amount * totalCount;
        end
        return result;
    end
    ...
    if adjType then
        ...
        if adjList then
            ...
            if count then
                local tilesRequired = tonumber(arguments.TilesRequired) or 1;
                local amount = tonumber(arguments.Amount) or 0;
                result.Amount = amount * math.floor(count / tilesRequired);
            end
        end
        return result;
    end
